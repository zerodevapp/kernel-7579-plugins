...
import "../src/WeightedValidator.sol";
...
contract WeightedValidatorTest is KernelTestBase {
    ...
    function _setRootValidationConfig() internal override {
        ...
        weightedValidator = new WeightedValidator();
        (x, y) = generatePublicKey(ownerKey);
        rootValidation = ValidatorLib.validatorToIdentifier(weightedValidator);
        bool usePrecompiled = generateUsePrecompiled(false);
        bytes[] memory arr = new bytes[](1);
        arr[0] = abi.encodePacked(
            bytes1(0x02), uint24(10), abi.encode(x, y, bytes32(keccak256(abi.encodePacked("Hello World"))))
        );
        rootValidationConfig = RootValidationConfig({
            hook: IHook(address(0)),
            hookData: hex"",
            validatorData: abi.encodePacked(uint24(10), uint48(0), abi.encode(arr))
        });
    }
    ...
    function _rootSignDigest(bytes32 digest, bool success) internal view override returns (bytes memory data) {
        ...
        arr[0] = abi.encodePacked(bytes1(0x00), sig);
